    ...
    return (
        <section id="destinations" className="px-20 py-10">
            ...
            <Carousel opts={{align:"start"}} className="w-full max-w-small pt-5 border-none" plugins={[plugin.current]} onMouseEnter={()=>plugin.current.stop()} onMouseLeave={()=>plugin.current.play()}>
                ...
                {destinationsData.map(({id, image, title, price, rating, ratingCount, emoji})=>(
                    <CarouselItem key={id} className="md:basis-1/2 lg:basis-1/3 pl-10 border-none m-0">
                    <div className="p-5">
                        <Card className="border-none rounded-3xl" style={{backgroundColor:"transparent", boxShadow: '10px 10px 20px rgba(0, 0, 0, 0.15)'}}>
                        <div className="w-full">
                            <Image src={image} alt="snow"/> 
                        </div>
                        <div className="-mt-3">
                            <Badge className="border-none shadow-lg ml-3">
                            <FaStar className="text-yellow-400 text-base"/>
                            <div className="flex pt-1 pl-1">
                                <p className="pt-1 pl-1">{rating}.0 <span className="text-gray-500">({ratingCount})</span></p>
                        ...
                        <div className="bg-white pt-12 pb-10 px-5 -mt-6 rounded-b-3xl">
                            ...
                            <p className="text-gray-400 text-sm font-medium"><span className="text-[#F57906]">{price}</span> per person</p>
                            </div>
                        </div>
                        </Card>
                    </div>
