    ...
    const destinationsData = [
        {
          ...
          name: "Max Skshena",  
          origin: "from USA",
          comment: "I recently used this app on my first trip to Estonia and was absolutely impressed with the experience. The app was user-friendly, making it incredibly easy to customize our tour based on my interests.",
        },
        {
            id:2,
    ...
    <section className="w-full px-24 pt-14 flex flex-col align-middle justify-center" id='testimonials'>
        <div className='flex flex-row justify-between'>
            <div>
                ...
                <h1 className="font-bold text-3xl leading-relaxed">Client Testimonial</h1>
            </div>
            <div className='absolute pl-[70%]'>
                <Image alt='helicopter' className='w-[200%]' src={TestimonialPlane}/>
            </div>
         </div>
         <Carousel opts={{align:"start"}} className="w-full max-w-small pt-5 border-none" plugins={[plugin.current]} onMouseEnter={()=>plugin.current.stop()} onMouseLeave={()=>plugin.current.play()}>
            <CarouselContent className="-ml-10 border-none">
                ...
                <CarouselItem key={id} className="pl-10 border-none m-0"> 
                    <div className='pt-5 pl-[10%] flex'>
                        <div className="w-[35%]">
                            <Image src={image} className='flex float-right rounded-3xl' style={{boxShadow: '-10px 10px 20px rgba(0, 0, 0, 0.15)'}} alt="snow"/>
                        </div>
            ...
            <CarouselPrevious style={{backgroundColor:"#F57906", border:"none"}} className="text-white"/>
            <CarouselNext style={{backgroundColor:"#F57906", border:"none"}} className="text-white"/>
        </Carousel>
    </section>
  )
}
